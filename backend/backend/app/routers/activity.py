...
from ..models import Pet, ActivityLog, NutritionPlan, User
from ..schemas import ActivitySync, ActivityCreate, ActivityEstimateCreate, ActivityResponse, NutritionPlanResponse
...
def _estimate_activity(activity_type: str, duration_minutes: int, intensity: int) -> tuple[int, int]:
    """Convert human-friendly activity to (steps, active_minutes).

    This is intentionally a simple heuristic so the feature works without wearables.
    """
    duration_minutes = max(0, int(duration_minutes or 0))
    intensity = max(1, min(5, int(intensity or 3)))

    # Baseline steps/minute by activity type
    steps_per_min = {
        "none": 0,
        "walk_slow": 80,
        "walk_normal": 100,
        ...
        "training": 60,
        "run": 140,
    }.get((activity_type or "").strip().lower(), 100)

    intensity_mult = {1: 0.7, 2: 0.85, 3: 1.0, 4: 1.15, 5: 1.3}[intensity]
    steps = int(round(steps_per_min * duration_minutes * intensity_mult))
    active_minutes = duration_minutes
    return max(0, steps), max(0, active_minutes)
...
def adjust_meal_for_activity(
    ...
    # --- Upgrade #1: Persist daily plan (upsert by pet_id + plan_date) ---
    existing_plan = (
        db.query(NutritionPlan)
        .filter(NutritionPlan.pet_id == pet.id, NutritionPlan.plan_date == target_date)
        .first()
    )
    if existing_plan:
        existing_plan.daily_calories = float(adjusted_calories)
        existing_plan.meal_frequency = int(meal_plan["meal_frequency"])
        existing_plan.portion_size_grams = float(meal_plan["grams_per_meal"])
        existing_plan.food_type = "kibble"
        existing_plan.notes = "Auto-saved from activity adjustment"
        plan = existing_plan
    else:
        plan = NutritionPlan(
            pet_id=pet.id,
            plan_date=target_date,
            daily_calories=float(adjusted_calories),
            meal_frequency=int(meal_plan["meal_frequency"]),
            portion_size_grams=float(meal_plan["grams_per_meal"]),
            food_type="kibble",
            notes="Auto-saved from activity adjustment",
        )
        db.add(plan)
...
def list_nutrition_plans(
    ...
    rows = (
        db.query(NutritionPlan)
        .filter(NutritionPlan.pet_id == pet.id)
        .order_by(NutritionPlan.plan_date.desc())
        .limit(limit)
        .all()
    )
    out: list[NutritionPlanResponse] = []
    for r in rows:
        total_grams = float(r.portion_size_grams) * int(r.meal_frequency)
        out.append(
            NutritionPlanResponse(
                ...
                meal_frequency=int(r.meal_frequency),
                portion_size_grams=float(r.portion_size_grams),
                food_type=r.food_type,
                notes=r.notes,
                total_grams_per_day=round(total_grams, 1),
            )
        )
    return out


# --- Upgrade #2: Activity estimator ---
@router.post("/estimate", response_model=ActivityResponse)
def estimate_and_log_activity(
    payload: ActivityEstimateCreate,
    user: User = Depends(require_user),
    db: Session = Depends(get_db),
):
    pet = _ensure_pet_owner(db, payload.pet_id, user)
    ...
    steps, active_minutes = _estimate_activity(payload.activity_type, payload.duration_minutes, payload.intensity)
    calories_burned = _calc_calories(steps, active_minutes)
